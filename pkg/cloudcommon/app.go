...
func ServeForever(app *appsrv.Application, options *CommonOptions, onStop func()) {
	AppDBInit(app)
	...
	if options.EnableSsl {
		...
		app.ListenAndServeTLS(addr, certfile, options.SslKeyfile, onStop)
	} else {
		app.ListenAndServe(addr, onStop)
	}
}
