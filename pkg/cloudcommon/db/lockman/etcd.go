...
func (rec *SEtcdLockRecord) unlockContext(ctx context.Context) (needClean bool) {
	...
	if rec.depth <= 0 {
		...
		// other players can proceed from this.
		unlocked := false
		for i := 0; i < 7; i++ {
			if err := rec.em.Unlock(ctx); err != nil {
				log.Errorf("%s: etcd unlock: %s", rec.pfx, err)
		...
		if !closed || !unlocked {
			log.Errorf("%s: session %v, lock %v", rec.pfx, closed, unlocked)
		}
		// NOTE Give the context a chance to recover by lock then
		// unlock again. Or do we need a cleaner to be tough and try
		// deleting the record to the eternal
