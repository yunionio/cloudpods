...
var ubuntu = SAWSImagePublisherInfo{
	...
	GetOSDist: func(image SImage) string {
		...
		if strings.HasPrefix(image.ImageName, "ubuntu-rolling-") || strings.HasPrefix(image.ImageName, "ubuntu-core") {
			return "Ubuntu Core"
		} else {
			return "Ubuntu"
		}
...
var (
	...
	amazonVersionPattern2 = regexp.MustCompile(`-(\d+)\.(\d+)\.(\d{8})(\.(\d+))?`)
)
...
var awsImagePublishers = map[string]SAWSImagePublisherInfo{
	...
	"280032941352": coreos,
	"595879546273": coreos,
	"016951021795": windowsServer,
	"801119661308": windowsServer,
	"137112412989": amazon,
	"141808717104": amazon,
}
