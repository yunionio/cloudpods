...
func (s *SLVMStorage) CreateDiskFromBackup(ctx context.Context, disk IDisk, input *SDiskCreateByDiskinfo) error {
	...
	newImg, err := qemuimg.NewQemuImage(disk.GetPath())
	if err != nil {
		return errors.Wrapf(err, "NewQemuImage(%s)", destImgPath)
	}
	if info.Encryption {
	...
	srcInfo := qemuimg.SImageInfo{
		...
		Password:      info.EncryptInfo.Key,
		EncryptFormat: qemuimg.EncryptFormatLuks,
		EncryptAlg:    info.EncryptInfo.Alg,
	}
...
func (s *SLVMStorage) CloneDiskFromStorage(
	...
	if fullCopy {
		_, err = srcImg.Clone(accessPath, qemuimg.QCOW2, false)
	} else {
