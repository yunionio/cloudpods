...
func (r *SRemoteFile) Fetch(callback func(progress, progressMbps float32, totalSizeMb int64)) error {
...
func (r *SRemoteFile) VerifyIntegrity(callback func(progress, progressMbps float32, totalSizeMb int64)) error {
...
func (r *SRemoteFile) fetch(preChksum string, callback func(progress, progressMbps float32, totalSizeMb int64)) error {
	var err error
	for i := 0; i < 3; i++ {
...
// retry download
func (r *SRemoteFile) download(getData bool, preChksum string, callback func(progress, progressMbps float32, totalSizeMb int64)) error {
	if getData {
...
func (r *SRemoteFile) downloadInternal(getData bool, preChksum string, callback func(progress, progressMbps float32, totalSize int64)) error {
				...
				for {
					...
					case <-time.After(1 * time.Second):
						...
						if callback != nil && percent > 0 {
							callback(float32(percent), float32(info.Size()-preSize)/1024/1024, totalSize/1024/1024)
						}
						preSize = info.Size()
					case <-finishChan:
						if callback != nil {
