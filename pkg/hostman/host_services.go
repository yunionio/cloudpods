...
func (host *SHostService) RunService() {
	...
	err = qemuimg.SetPreallocation(options.HostOptions.Qcow2Preallocation)
	if err != nil {
		log.Fatalf("failed set qemuimg preallocation to %s", options.HostOptions.Qcow2Preallocation)
	}
