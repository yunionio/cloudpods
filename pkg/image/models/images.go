	...
	for fsid, storageIds := range cephStorages.CephFsidStorageId {
		...
		for storageId := range storageCachedImages {
			if storageId != cachedRbdimgStorageId && storageCachedImages[storageId] == nil {
				...
				err := img.cloneToCephStorage(ctx, srcConf.MonHost, srcConf.Key, srcConf.Pool, destConf.Pool)
				if err != nil {
...
func (img *SImage) removeCephImage(storageConf *computeapi.RbdStorageConf, imgName string) error {
	cli, err := cephutils.NewClient(storageConf.MonHost, storageConf.Key, storageConf.Pool)
	if err != nil {
...
func (img *SImage) cloneToCephStorage(ctx context.Context, monHost, key, pool string, destPool string) error {
	imgName := "image_cache_" + img.Id
	cli, err := cephutils.NewClient(monHost, key, pool)
	if err != nil {
		return errors.Wrap(err, "cephutils.NewClient")
	}
	defer cli.Close()
