...
// the same order applies to the rules of a guest network, which may add
// security groups bound to the network interface
func TestOrderedGuestnetworkSecurityGroupRulesAreSortedByPriority(t *testing.T) {
	guest := &Guest{}
	gn := &Guestnetwork{}
	gn.Guestnetworksecgroups = Guestnetworksecgroups{
		"gnsg1": &Guestnetworksecgroup{
			SecurityGroup: newTestSecgroup(
				newTestSecgroupRule("r30", 30),
				newTestSecgroupRule("r10", 10),
				newTestSecgroupRule("r20", 20),
			),
	...
	rules := gn.OrderedSecurityGroupRules(guest)
	if len(rules) != 2+3 {
		t.Fatalf("got %d rules, want %d", len(rules), 2+3)
	}
	if rules[0].Priority != 1 || rules[1].Priority != 2 {
		t.Fatalf("the prefixes are missing: %d, %d", rules[0].Priority, rules[1].Priority)
	}
	for i := 1; i < len(rules); i++ {
		if rules[i-1].Priority > rules[i].Priority {
			t.Fatalf("rule %s (priority %d) comes before rule %s (priority %d)",
