...
type ServerConfig struct {
	Host string `mapstructure:"host"` // 服务监听地址（默认：localhost）
	Port int    `mapstructure:"port"` // 服务监听端口
}

type MCPConfig struct {
	Name        string `mapstructure:"name"`        // MCP 服务名称
	Version     string `mapstructure:"version"`     // MCP 服务版本
	Description string `mapstructure:"description"` // MCP 服务描述
}
...
type CloudpodsConfig struct {
	BaseURL string `mapstructure:"base_url" yaml:"base_url"` // 认证服务入口
...
func Load(configPath string) (*Config, error) {
	...
	v.SetDefault("external.cloudpods.base_url", "https://localhost/api/s/identity/v3")
