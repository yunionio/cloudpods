...
func (obj *SDevtoolTemplate) PreDelete(ctx context.Context, userCred mcclient.TokenCredential) {

	task, err := taskman.TaskManager.NewTask(ctx, "TemplateDelete", obj, userCred, nil, "", "", nil)
	if err != nil {
		log.Errorf("register task TemplateDelete error %s", err)
		return
	}
	task.ScheduleRun(nil)
}
