...
func (cliss *ClientSession) GetServiceVersionURLsByMethod(service, endpointType string, method httputils.THttpMethod) ([]string, error) {
	...
	if endpointType == api.EndpointInterfaceApigateway {
		...
		var mode api.TEndpointMode
		switch method {
		case httputils.GET, httputils.HEAD:
			mode = api.EndpointModeSlave
		default:
			mode = api.EndpointModeNormal
		}
		return cliss.getServiceVersionURLs(service, cliss.region, cliss.zone, endpointType, mode)
	}
}

func (cliss *ClientSession) getApigatewayServiceURLs(service, region, zone string) ([]string, error) {
	urls, err := cliss.getServiceVersionURLs(service, region, zone, api.EndpointInterfaceInternal, api.EndpointModeNormal)
	if err != nil {
...
func (cliss *ClientSession) getServiceVersionURLs(service, region, zone, endpointType string, endpointMode api.TEndpointMode) ([]string, error) {
	...
	if service == api.SERVICE_TYPE && endpointMode == api.EndpointModeNormal && (err != nil || len(urls) == 0 || (len(cliss.client.authUrl) != 0 && cliss.client.authUrl[:5] != urls[0][:5])) {
		var msg string
		if err != nil {
