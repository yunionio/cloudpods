...
func NewContainerRegistryManager() *ContainerRegistryManager {
	man := &ContainerRegistryManager{
		ResourceManager: modules.NewImageManager("container_registry", "container_registries",
			[]string{"ID", "Name", "Url", "Type", "Status"},
			[]string{}),
	}
	// Force /container_registries/<id>/images to GetSpecific, otherwise apigateway
	// treats "images" as the glance images module and does ListInContext.
...
func (m *ContainerRegistryManager) UploadImage(s *mcclient.ClientSession, id string, params jsonutils.JSONObject, body io.Reader, size int64) (jsonutils.JSONObject, error) {
	...
	resp, err := modulebase.RawRequest(m.ResourceManager, s, httputils.POST, path, headers, body)
