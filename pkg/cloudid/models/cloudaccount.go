...
func (m *SCloudaccountManager) OnDelete(obj *jsonutils.JSONDict) {
	...
	err = account.purge(context.Background(), auth.AdminCredential())
	if err != nil {
...
func (m *SCloudaccountManager) OnUpdate(oldObj, newObj *jsonutils.JSONDict) {
	oldV, _ := oldObj.Bool("saml_auth")
	newV, _ := newObj.Bool("saml_auth")
	log.Debugf("OnUpdate old saml auth: %v new saml auth: %v", oldV, newV)
	if !oldV && newV {
		err := func() error {
			id, err := newObj.GetString("id")
			if err != nil {
				return errors.Wrapf(err, "get id")
			}
			account, err := m.FetchAccount(id)
			if err != nil {
				return errors.Wrapf(err, "FetchAccount")
			}
			return account.StartSAMLProviderCreateTask(context.Background(), auth.AdminCredential())
		}()
		if err != nil {
			log.Errorf("on update error: %v", err)
		}
	}
}
