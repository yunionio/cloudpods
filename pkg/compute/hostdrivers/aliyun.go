...
func (self *SAliyunHostDriver) ValidateCreateDisk(storage *models.SStorage, sizeGb int) error {
	if utils.IsInStringArray(storage.StorageType, []string{models.STORAGE_CLOUD_EFFICIENCY, models.STORAGE_CLOUD_SSD, models.STORAGE_CLOUD_ESSD}) {
		if sizeGb < 20 || sizeGb > 32768 {
			return fmt.Errorf("The %s disk size must be in the range of 20G ~ 32768G", storage.StorageType)
		}
	} else if storage.StorageType == models.STORAGE_PUBLIC_CLOUD {
		if sizeGb < 5 || sizeGb > 2000 {
			return fmt.Errorf("The %s disk size must be in the range of 5G ~ 2000G", storage.StorageType)
		}
	} else {
