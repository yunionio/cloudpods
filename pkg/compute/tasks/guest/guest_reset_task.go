...
func (self *GuestHardResetTask) StopServer(ctx context.Context, guest *models.SGuest) {
	...
	guest.StartGuestStopTask(ctx, self.UserCred, nil, true, false, self.GetTaskId())
...
func (self *GuestRestartTask) StopServer(ctx context.Context, guest *models.SGuest) {
	...
	var timeout *int
	timeoutSec, err := self.Params.Int("timeout")
	if err == nil {
		intTimeout := int(timeoutSec)
		timeout = &intTimeout
	}
	guest.StartGuestStopTask(ctx, self.UserCred, timeout, isForce, false, self.GetTaskId())
