...
func (task *GuestBatchCreateTask) GetSchedParams() (*schedapi.ScheduleInput, error) {
	params := taskutils.GetBatchParamsAtIndex(task, 0)
...
func (task *GuestBatchCreateTask) clearPendingUsage(ctx context.Context, guest *models.SGuest) {
	taskutils.ClearTaskPendingUsage(ctx, task)
	taskutils.ClearTaskPendingRegionUsage(ctx, task)
}

func (task *GuestBatchCreateTask) OnInit(ctx context.Context, objs []db.IStandaloneModel, body jsonutils.JSONObject) {
	taskutils.StartScheduleObjects(ctx, task, objs)
}

func (task *GuestBatchCreateTask) OnScheduleFailCallback(ctx context.Context, obj taskutils.IScheduleModel, reason jsonutils.JSONObject, index int) {
...
func (task *GuestBatchCreateTask) SaveScheduleResultWithBackup(ctx context.Context, obj taskutils.IScheduleModel, master, slave *schedapi.CandidateResource, index int) {
	guest := obj.(*models.SGuest)
...
func (task *GuestBatchCreateTask) SaveScheduleResult(ctx context.Context, obj taskutils.IScheduleModel, candidate *schedapi.CandidateResource, index int) {
	var err error
	...
	data := taskutils.GetBatchParamsAtIndex(task, index)
