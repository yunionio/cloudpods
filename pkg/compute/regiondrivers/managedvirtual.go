...
func (self *SManagedVirtualizationRegionDriver) RequestSyncSecurityGroup(ctx context.Context, userCred mcclient.TokenCredential, managerId, vpcId string, vpc *models.SVpc, secgroup *models.SSecurityGroup, remoteProjectId, service string, skipSyncRule bool) (string, error) {
	lockman.LockRawObject(ctx, "secgroupcache", fmt.Sprintf("%s-%s-%s", secgroup.Id, vpcId, managerId))
	defer lockman.ReleaseRawObject(ctx, "secgroupcache", fmt.Sprintf("%s-%s-%s", secgroup.Id, vpcId, managerId))
...
func (self *SManagedVirtualizationRegionDriver) RequestCacheSecurityGroup(ctx context.Context, userCred mcclient.TokenCredential, region *models.SCloudregion, vpc *models.SVpc, secgroup *models.SSecurityGroup, classic bool, removeProjectId string, task taskman.ITask) error {
	...
	taskman.LocalTaskRun(task, func() (jsonutils.JSONObject, error) {
		_, err := self.RequestSyncSecurityGroup(ctx, userCred, managerId, vpcId, vpc, secgroup, removeProjectId, "", false)
		return nil, err
	})
	return nil
}
...
func (self *SManagedVirtualizationRegionDriver) RequestCreateDBInstance(ctx context.Context, userCred mcclient.TokenCredential, dbinstance *models.SDBInstance, task taskman.ITask) error {
	taskman.LocalTaskRun(task, func() (jsonutils.JSONObject, error) {
		...
		for i := range secgroups {
			...
			secId, err := region.GetDriver().RequestSyncSecurityGroup(ctx, userCred, _cloudprovider.Id, vpcId, vpc, &secgroups[i], desc.ProjectId, "", true)
			if err != nil {
...
func (self *SManagedVirtualizationRegionDriver) RequestSyncSecgroupsForElasticcache(ctx context.Context, userCred mcclient.TokenCredential, ec *models.SElasticcache, task taskman.ITask) error {
	taskman.LocalTaskRun(task, func() (jsonutils.JSONObject, error) {
		...
		secgroupExternalIds := []string{}
		{
			...
			for i := range ess {
				externalId, err := self.RequestSyncSecurityGroup(ctx, task.GetUserCred(), provider.Id, vpcId, vpc, ess[i].GetSecGroup(), "", "redis", true)
				if err != nil {
...
func (self *SManagedVirtualizationRegionDriver) RequestSyncRdsSecurityGroups(ctx context.Context, userCred mcclient.TokenCredential, rds *models.SDBInstance, task taskman.ITask) error {
	taskman.LocalTaskRun(task, func() (jsonutils.JSONObject, error) {
		...
		secgroupIds := []string{}
		for i := range secgroups {
			secgroupId, err := self.RequestSyncSecurityGroup(ctx, userCred, provider.Id, vpc.ExternalId, vpc, &secgroups[i], iRds.GetProjectId(), "rds", true)
			if err != nil {
