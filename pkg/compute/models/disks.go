...
func (self *SDisk) syncWithCloudDisk(ctx context.Context, userCred mcclient.TokenCredential, provider cloudprovider.ICloudProvider, extDisk cloudprovider.ICloudDisk, index int, syncOwnerId mcclient.IIdentityProvider, managerId string) error {
	...
	diff, err := db.UpdateWithLock(ctx, self, func() error {
		...
		if templateId := extDisk.GetTemplateId(); len(templateId) > 0 {
			cachedImage, err := db.FetchByExternalIdAndManagerId(CachedimageManager, templateId, func(q *sqlchemy.SQuery) *sqlchemy.SQuery {
				storage := StorageManager.Query().SubQuery()
				sci := StoragecachedimageManager.Query().SubQuery()
				return q.Join(sci, sqlchemy.Equals(sci.Field("cachedimage_id"), q.Field("id"))).
					Join(storage, sqlchemy.Equals(storage.Field("storagecache_id"), sci.Field("storagecacheId"))).
					Filter(sqlchemy.Equals(storage.Field("manager_id"), managerId))
			})
