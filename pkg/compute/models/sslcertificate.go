...
func (r *SCloudregion) GetSSLCertificates(managerId string) ([]SSSLCertificate, error) {
	//q := SSLCertificateManager.Query().Equals("cloudregion_id", r.Id)
	q := SSLCertificateManager.Query()
	if len(managerId) > 0 {
...
func (r *SCloudregion) SyncSSLCertificates(
	ctx context.Context,
	userCred mcclient.TokenCredential,
	...
	// 加锁防止重入
	lockman.LockRawObject(ctx, SSLCertificateManager.KeywordPlural(), fmt.Sprintf("%s-%s", provider.Id, r.Id))
	defer lockman.ReleaseRawObject(ctx, SSLCertificateManager.KeywordPlural(), fmt.Sprintf("%s-%s", provider.Id, r.Id))

	result := compare.SyncResult{}

	dbEss, err := r.GetSSLCertificates(provider.Id)
	if err != nil {
		result.Error(err)
		return result
	}
	...
	for i := 0; i < len(added); i++ {
		_, err := r.newFromCloudSSLCertificate(ctx, userCred, provider, added[i])
		if err != nil {
			result.AddError(err)
			continue
		}
...
func (r *SCloudregion) newFromCloudSSLCertificate(
	ctx context.Context,
	userCred mcclient.TokenCredential,
	provider *SCloudprovider,
	...
	err = func() error {
		...
		s.Name, err = db.GenerateName(ctx, SSLCertificateManager, provider.GetOwnerId(), ext.GetName())
		if err != nil {
