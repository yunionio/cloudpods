...
func (spcm *SSnapshotPolicyCacheManager) ListItemFilter(ctx context.Context, q *sqlchemy.SQuery, userCred mcclient.TokenCredential,
	...
	if defsecgroup, _ := query.GetString("snapshotpolicy.yaml"); len(defsecgroup) > 0 {
...
func (spc *SSnapshotPolicyCache) GetIRegion() (cloudprovider.ICloudRegion, error) {
	...
	return nil, fmt.Errorf("failed to find iregion for snapshotpolicycache %s: cloudregion %d manager %d", spc.Id,
...
func (spc *SSnapshotPolicyCache) GetSnapshotPolicy() (*SSnapshotPolicy, error) {
	...
	if err != nil {
		return nil, fmt.Errorf("failed to fetch snapshotpolicy.yaml by %s", spc.SnapshotpolicyId)
	}
...
func (spcm *SSnapshotPolicyCacheManager) NewCache(ctx context.Context, userCred mcclient.TokenCredential,
	...
	lockman.LockClass(ctx, spcm, userCred.GetProjectId())
	defer lockman.ReleaseClass(ctx, spcm, userCred.GetProjectId())
...
func (spcm *SSnapshotPolicyCacheManager) DeleteCloudSnapshotPolices(ctx context.Context,
	...
	if err != nil {
		return errors.Wrapf(err, "fetch all snapshotPolicyCaches of snapshotpolicy.yaml %s failed", snapshotPolicyId)
	}
...
func (spc *SSnapshotPolicyCache) CreateCloudSnapshotPolicy() error {
	// create corresponding snapshotpolicy.yaml in cloud
	...
	if err != nil {
		return errors.Wrap(err, "create snapshotpolicy.yaml failed")
	}
	spc.ExternalId = externalId
