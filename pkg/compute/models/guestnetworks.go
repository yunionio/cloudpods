...
func (gn *SGuestnetwork) getJsonDesc() *api.GuestnetworkJsonDesc {
	...
	desc.RxBwLimit = gn.getRxBwLimit()
	desc.TxBwLimit = gn.getTxBwLimit()
...
func (gn *SGuestnetwork) GetDetailedString() string {
	...
	parts = append(parts,
		...
		fmt.Sprintf("rx%d", gn.getRxBwLimit()),
		fmt.Sprintf("tx%d", gn.getTxBwLimit()),
...
func (gn *SGuestnetwork) getRxBwLimit() int {
	if gn.RxBwLimit > 0 && gn.RxBwLimit <= api.MAX_BANDWIDTH {
		return gn.RxBwLimit
	}
	return gn.getBandwidth()
}

func (gn *SGuestnetwork) getTxBwLimit() int {
