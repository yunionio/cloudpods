...
func (s *STopic) PerformAddResource(ctx context.Context, userCred mcclient.TokenCredential, query jsonutils.JSONObject, input api.STopicResourceCreateInput) (jsonutils.JSONObject, error) {
	...
	count, err = TopicResourceManager.Query().Equals("resource_id", input.ResourceId).Equals("topic_id", input.TopicId).CountWithError()
	if err != nil {
		return nil, errors.Wrap(err, "fetch count")
	}
	if count > 0 {
		return nil, errors.Wrapf(httperrors.ErrDuplicateResource, "topic:%s,resource:%s has been exist", input.TopicId, input.ResourceId)
	}
	return nil, nil
}

func (s *STopic) PerformAddAction(ctx context.Context, userCred mcclient.TokenCredential, query jsonutils.JSONObject, input api.STopicActionCreateInput) (jsonutils.JSONObject, error) {
	...
	count, err = TopicActionManager.Query().Equals("action_id", input.ActionId).Equals("topic_id", input.TopicId).CountWithError()
	if err != nil {
		return nil, errors.Wrap(err, "fetch count")
	}
	if count > 0 {
		return nil, errors.Wrapf(httperrors.ErrDuplicateResource, "topic:%s,action:%s has been exist", input.TopicId, input.ActionId)
	}
	return nil, nil
}
