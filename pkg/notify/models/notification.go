...
type SNotification struct {
	...
	ContactType string `width:"128" nullable:"true" create:"optional" search:"user"`
	// swagger:ignore
...
func (nm *SNotificationManager) ValidateCreateData(ctx context.Context, userCred mcclient.TokenCredential, ownerId mcclient.IIdentityProvider, query jsonutils.JSONObject, input api.NotificationCreateInput) (api.NotificationCreateInput, error) {
	...
	for i := range input.Receivers {
		receiver, err := validators.ValidateModel(userCred, ReceiverManager, &input.Receivers[i])
		if err != nil && !input.IgnoreNonexistentReceiver {
			return input, err
		}
		if receiver != nil && !utils.IsInStringArray(receiver.GetId(), receivers) {
			receivers = append(receivers, receiver.GetId())
		}
	}
...
func (nm *SNotificationManager) PerformEventNotify(ctx context.Context, userCred mcclient.TokenCredential, query jsonutils.JSONObject, input api.NotificationManagerEventNotifyInput) (api.NotificationManagerEventNotifyOutput, error) {
	output := api.NotificationManagerEventNotifyOutput{}
	if len(input.ResourceType) == 0 {
		return output, httperrors.NewMissingParameterError("resource_type")
	}
	if len(input.Action) == 0 {
		return output, httperrors.NewMissingParameterError("action")
	}
	for _, cType := range input.ContactTypes {
		driver := GetDriver(cType)
		if driver == nil {
			return output, httperrors.NewUnsupportOperationError("unsupport contact type %s", cType)
		}
	}

	topic, err := TopicManager.GetTopicByEvent(input.ResourceType, input.Action, input.AdvanceDays)
	if err != nil {
		return output, err
	}

	return output, topic.StartMessageSendTask(ctx, userCred, input)

	/*
		var receiverIds []string
		receiverIds1, err := SubscriberManager.getReceiversSent(ctx, topics[0].Id, input.ProjectDomainId, input.ProjectId)
		if err != nil {
		...
		// robot
		var robots []string
		_robots, err := SubscriberManager.robot(topics[0].Id, input.ProjectDomainId, input.ProjectId)
		if err != nil {
			if errors.Cause(err) != errors.ErrNotFound {
				return output, errors.Wrapf(err, "unable fetch robot of subscription %q", topics[0].Id)
			}
		} else {
		...
		// create event
		event, err := EventManager.CreateEvent(ctx, input.Event, topics[0].Id, message, input.AdvanceDays)
		if err != nil {
		...
		// normal contact type
		for _, ct := range input.ContactTypes {
			if ct == api.MOBILE {
				continue
			}
...
func (n *SNotification) GetNotOKRobots() ([]SRobot, error) {
	ret := []SRobot{}
	q := RobotManager.Query()
	sq := ReceiverNotificationManager.Query().Equals("notification_id", n.Id).NotEquals("status", api.RECEIVER_NOTIFICATION_OK).Equals("receiver_type", api.RECEIVER_TYPE_ROBOT).SubQuery()
	q = q.Join(sq, sqlchemy.Equals(q.Field("id"), sq.Field("receiver_id")))
	err := db.FetchModelObjects(RobotManager, q, &ret)
	return ret, err
}
