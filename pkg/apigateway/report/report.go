...
func Report(ctx context.Context, userCred mcclient.TokenCredential, isStart bool) {
	getFunc := func() (*sReport, error) {
		...
		user, err := modules.UsersV3.Get(s, idapi.SystemAdminUser, nil)
		if err != nil {
		...
		resp, err := modules.Hosts.List(s, jsonutils.Marshal(map[string]interface{}{
			"scope":      "system",
		...
		resp, err = modules.Capabilities.List(s, system)
		if err != nil {
		...
		usage, err := modules.Usages.GetGeneralUsage(s, system)
		if err != nil {
		...
		usage, err = modules.IdentityUsages.GetUsage(s, system)
		if err != nil {
