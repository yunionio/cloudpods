...
func (self *HcsOpCollect) CollectWireMetrics(ctx context.Context, manager api.CloudproviderDetails, provider cloudprovider.ICloudProvider, res map[string]api.WireDetails, start, end time.Time) error {
	...
	for i := range res {
		ch <- struct{}{}
		wg.Add(1)
		go func(wire api.WireDetails) {
			defer func() {
				wg.Done()
				<-ch
			}()
			opts := &cloudprovider.MetricListOptions{
				ResourceType: cloudprovider.METRIC_RESOURCE_TYPE_WIRE,
				StartTime:    start,
				EndTime:      end,
			}
			opts.ResourceId = wire.ExternalId

			tags := []influxdb.SKeyValue{}
			for k, v := range wire.GetMetricTags() {
			...
			if err != nil {
				if errors.Cause(err) != cloudprovider.ErrNotImplemented && errors.Cause(err) != cloudprovider.ErrNotSupported {
					log.Errorf("get rds %s(%s) error: %v", wire.Name, wire.Id, err)
					return
				}
				return
			}
	...
	return self.sendMetrics(ctx, manager, "wire", len(res), metrics)
}
