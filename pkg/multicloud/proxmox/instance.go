...
func (self *SInstance) GetIDisks() ([]cloudprovider.ICloudDisk, error) {
	...
	for _, v := range self.QemuDisks {
		disk, err := self.host.zone.region.GetDisk(v["volume"].(string))
		if err != nil {
			return nil, err
		}
		ret = append(ret, disk)
	...
	for _, v := range self.QemuUnusedDisks {
		disk, err := self.host.zone.region.GetDisk(v["volume"].(string))
		if err != nil {
			return nil, err
		}
...
func (self *SRegion) GetQemuConfig(node string, VmId int) (*SInstance, error) {
	...
	// cores:2 ostype:l26
	name := ""
	if _, ok := vmConfig["name"]; ok {
		name = vmConfig["name"].(string)
	}
	description := ""
	if _, ok := vmConfig["description"]; ok {
		description = vmConfig["description"].(string)
	}
	tags := ""
	if _, ok := vmConfig["tags"]; ok {
		tags = vmConfig["tags"].(string)
	}
	args := ""
	if _, ok := vmConfig["args"]; ok {
		args = vmConfig["args"].(string)
	}
	bios := "seabios"
	if _, ok := vmConfig["bios"]; ok {
		bios = vmConfig["bios"].(string)
	}
	onboot := true
	if _, ok := vmConfig["onboot"]; ok {
		onboot = Itob(int(vmConfig["onboot"].(int64)))
	}
	startup := ""
	if _, ok := vmConfig["startup"]; ok {
		startup = vmConfig["startup"].(string)
	}
	tablet := true
	if _, ok := vmConfig["tablet"]; ok {
		tablet = Itob(int(vmConfig["tablet"].(int64)))
	}
	agent := 0
	...
	ostype := "other"
	if _, ok := vmConfig["ostype"]; ok {
		ostype = vmConfig["ostype"].(string)
	}
	var memory int64 = 0
	if _, ok := vmConfig["memory"]; ok {
		memory = vmConfig["memory"].(int64)
	}
	var balloon int64 = 0
	if _, ok := vmConfig["balloon"]; ok {
		balloon = vmConfig["balloon"].(int64)
	}
	var cores int64 = 1
	if _, ok := vmConfig["cores"]; ok {
		cores = vmConfig["cores"].(int64)
	}
	var vcpus int64 = 0.0
	if _, ok := vmConfig["vcpus"]; ok {
		vcpus = vmConfig["vcpus"].(int64)
	}
	var sockets int64 = 1.0
	if _, ok := vmConfig["sockets"]; ok {
		sockets = vmConfig["sockets"].(int64)
	}
	cpu := "host"
	if _, ok := vmConfig["cpu"]; ok {
		cpu = vmConfig["cpu"].(string)
	}
	numa := false
	if _, ok := vmConfig["numa"]; ok {
		numa = Itob(int(vmConfig["numa"].(int64)))
	}
	//Can be network,disk,cpu,memory,usb
	hotplug := "network,disk,usb"
	if _, ok := vmConfig["hotplug"]; ok {
		hotplug = vmConfig["hotplug"].(string)
	}
	//boot by default from hard disk (c), CD-ROM (d), network (n).
	boot := "cdn"
	if _, ok := vmConfig["boot"]; ok {
		boot = vmConfig["boot"].(string)
	}
	bootdisk := ""
	if _, ok := vmConfig["bootdisk"]; ok {
		bootdisk = vmConfig["bootdisk"].(string)
	}
	kvm := true
	if _, ok := vmConfig["kvm"]; ok {
		kvm = Itob(int(vmConfig["kvm"].(int64)))
	}
	scsihw := "lsi"
	if _, ok := vmConfig["scsihw"]; ok {
		scsihw = vmConfig["scsihw"].(string)
	}
	hookscript := ""
	if _, ok := vmConfig["hookscript"]; ok {
		hookscript = vmConfig["hookscript"].(string)
	}
	machine := "i440fx"
	if _, ok := vmConfig["machine"]; ok {
		machine = vmConfig["machine"].(string)
	}
...
func (self *SRegion) StartVm(vmId int) error {
	...
	nodeName := ""
	if resource, ok := resources[vmId]; !ok {
		return errors.Errorf("start VM id %s", vmId)
	} else {
		nodeName = resource.Node
	}
...
func (self *SRegion) AttachDisk(vmId int, diskId string) error {
	...
	if _, ok := vm1.QemuUnusedDisks[diskId]; !ok {
		...
		if err != nil {
			return errors.Wrapf(err, "GetInstance(%s) self.put", vmId)
		}
		//clear
		...
		if err != nil {
			return errors.Wrapf(err, "GetInstance(%s) vm2", vmId)
		}
...
func (self *SRegion) ResetVmPassword(vmId int, username, password string) error {
	...
	nodeName := ""
	if resource, ok := resources[vmId]; !ok {
		return errors.Errorf("failed to ResetVmPassword VM id %s", vmId)
	} else {
		nodeName = resource.Node
	}
...
func (self *SRegion) DeleteVM(vmId int) error {
	...
	nodeName := ""
	if resource, ok := resources[vmId]; !ok {
		return errors.Errorf("failed to delete VM id %s", vmId)
	} else {
		nodeName = resource.Node
	}
