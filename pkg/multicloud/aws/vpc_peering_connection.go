...
func (self *SRegion) GetVpcPeeringConnection(id string) (*SVpcPeeringConnection, error) {
	peers, err := self.DescribeVpcPeeringConnections(id, "", "")
	if err != nil {
		return nil, errors.Wrapf(err, "DescribeVpcPeeringConnections")
	}
	for i := range peers {
		if peers[i].GetGlobalId() == id {
			return &peers[i], nil
		}
	}
...
func (self *SRegion) DescribeVpcPeeringConnections(id, acceptVpcId, requestVpcId string) ([]SVpcPeeringConnection, error) {
	params := map[string]string{}
	idx := 1
	if len(acceptVpcId) > 0 {
		params[fmt.Sprintf("Filter.%d.accepter-vpc-info.vpc-id", idx)] = acceptVpcId
		idx++
	}
	if len(requestVpcId) > 0 {
		params[fmt.Sprintf("Filter.%d.requester-vpc-info.vpc-id", idx)] = requestVpcId
		idx++
	}
	if len(id) > 0 {
		params["VpcPeeringConnectionId.1"] = id
	}
	...
	for {
		...
		err := self.ec2Request("DescribeVpcPeeringConnections", params, &result)
		if err != nil {
			return nil, errors.Wrapf(err, "DescribeVpcPeeringConnections")
		}
		for i := range result.VpcPeeringConnectionSet {
			if result.VpcPeeringConnectionSet[i].Status.Code == "deleted" {
				continue
			}
			ret = append(ret, result.VpcPeeringConnectionSet[i])
		}
		if len(result.NextToken) == 0 || len(result.VpcPeeringConnectionSet) == 0 {
			break
		}
...
func (self *SRegion) CreateVpcPeeringConnection(vpcId string, opts *cloudprovider.VpcPeeringConnectionCreateOptions) (*SVpcPeeringConnection, error) {
	params := map[string]string{
		...
		"TagSpecification.1.ResoruceType": "vpc-peering-connection",
	...
	if len(opts.Desc) > 0 {
		...
		params["TagSpecification.1.Tags.2.Value"] = opts.Desc
	}
...
func (self *SRegion) DeleteVpcPeeringConnection(id string) error {
	...
	return self.ec2Request("DeleteVpcPeeringConnection", params, nil)
}

func (self *SRegion) AcceptVpcPeeringConnection(vpcPeeringConnectionId string) (*SVpcPeeringConnection, error) {
	params := map[string]string{
	...
	return &ret.VpcPeeringConnection, self.ec2Request("AcceptVpcPeeringConnection", params, &ret)
}

func (self *SRegion) DeleteVpcPeeringConnectionRoute(peerId string) error {
	routeTables, err := self.GetRouteTables("", peerId, "", nil, true)
	if err != nil {
		return errors.Wrapf(err, "GetRouteTables")
	}

	for i := range routeTables {
		for j := range routeTables[i].RouteSet {
			if routeTables[i].RouteSet[j].VpcPeeringConnectionId == peerId && len(routeTables[i].RouteSet[j].DestinationCidrBlock) > 0 {
				err = self.RemoveRoute(routeTables[i].RouteTableId, routeTables[i].RouteSet[j].DestinationCidrBlock)
				if err != nil {
...
// initiating-request | pending-acceptance | active | deleted | rejected | failed | expired | provisioning | deleting
...
func (self *SVpcPeeringConnection) Refresh() error {
	peer, err := self.vpc.region.GetVpcPeeringConnection(self.VpcPeeringConnectionId)
	if err != nil {
		return err
	}
