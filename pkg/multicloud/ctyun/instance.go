...
func (self *SInstance) RebuildRoot(ctx context.Context, imageId string, passwd string, publicKey string, sysSizeGB int) (string, error) {
	...
	jobId := ""
	if currentImage.GetId() != imageId {
		jobId, err = self.host.zone.region.SwitchVMOs(self.GetId(), passwd, publicKeyName, imageId)
		if err != nil {
			return "", errors.Wrap(err, "SInstance.RebuildRoot.SwitchVMOs")
		}
	} else {
		jobId, err = self.host.zone.region.RebuildVM(self.GetId(), passwd, publicKeyName)
		if err != nil {
