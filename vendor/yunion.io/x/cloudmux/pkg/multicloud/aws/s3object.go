...
func (o *SObject) GetAcl() cloudprovider.TBucketACLType {
	...
	output, err := s3cli.GetObjectAcl(context.Background(), input)
	if err != nil {
...
func (o *SObject) SetAcl(aclStr cloudprovider.TBucketACLType) error {
	...
	input := &s3.PutObjectAclInput{
		Bucket: &o.bucket.Name,
		Key:    &o.Key,
		ACL:    types.ObjectCannedACL(string(aclStr)),
	}
	_, err = s3cli.PutObjectAcl(context.Background(), input)
	if err != nil {
...
func (o *SObject) GetMeta() http.Header {
	...
	s3cli, err := o.bucket.region.GetS3Client()
	if err != nil {
		log.Errorf("GetS3Client fail %s", err)
		return nil
	}
	...
	output, err := s3cli.HeadObject(context.Background(), input)
	if err != nil {
