...
func (self *SHost) CreateVM(opts *cloudprovider.SManagedVMCreateConfig) (cloudprovider.ICloudVM, error) {
	...
	network, err := self.zone.region.GetNetwork(opts.ExternalNetworkId)
	if err != nil {
		return nil, errors.Wrapf(err, "GetNetwork")
	}
	if network.DhcpEnabled {
		nic.Dhcp = true
		nic.DhcpEnabled = true
		nic.DhcpIP = opts.IpAddr
	} else if len(opts.IpAddr) > 0 {
		nic.IP = opts.IpAddr
		mask := net.CIDRMask(int(network.GetIpMask()), 32)
		nic.Netmask = fmt.Sprintf("%d.%d.%d.%d", mask[0], mask[1], mask[2], mask[3])
		nic.Gateway = network.GetGateway()
		if len(nic.Gateway) == 0 {
			info := strings.Split(opts.IpAddr, ".")
			if len(info) == 4 {
				info[3] = "1"
				nic.Gateway = strings.Join(info, ".")
			}
		}
	}
