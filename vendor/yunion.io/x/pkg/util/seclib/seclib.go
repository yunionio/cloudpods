...
// randomIndex returns a uniformly random index in [0, n), read from the
// system random source.
//
// A failure of that source cannot be recovered from here: falling back to a
// deterministic value would hand out a predictable password, so the caller is
// stopped rather than served a weaker one.
func randomIndex(n int) int {
	if n <= 0 {
		panic(fmt.Sprintf("seclib: invalid random range %d", n))
	}
	if n == 1 {
		return 0
	}
	// Values at or above limit would make the modulo below uneven, so they
	// are drawn again instead.
	limit := 256 - (256 % n)
	buf := make([]byte, 1)
	for {
		if _, err := io.ReadFull(crand.Reader, buf); err != nil {
			panic(fmt.Sprintf("seclib: random source failed: %v", err))
		}
		if int(buf[0]) < limit {
			return int(buf[0]) % n
		}
