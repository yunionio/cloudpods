...
// BuildConfigFromFlags is a helper function that builds configs from a main
// url or a kubeconfig filepath. These are passed in as command line flags for cluster
// components. Warnings should reflect this usage. If neither mainUrl or kubeconfigPath
// are passed in we fallback to inClusterConfig. If inClusterConfig fails, we fallback
// to the default config.
func BuildConfigFromFlags(mainUrl, kubeconfigPath string) (*restclient.Config, error) {
	if kubeconfigPath == "" && mainUrl == "" {
		klog.Warningf("Neither --kubeconfig nor --main was specified.  Using the inClusterConfig.  This might not work.")
	...
	return NewNonInteractiveDeferredLoadingClientConfig(
		...
		&ConfigOverrides{ClusterInfo: clientcmdapi.Cluster{Server: mainUrl}}).ClientConfig()
}

// BuildConfigFromKubeconfigGetter is a helper function that builds configs from a main
// url and a kubeconfigGetter.
func BuildConfigFromKubeconfigGetter(mainUrl string, kubeconfigGetter KubeconfigGetter) (*restclient.Config, error) {
	...
	cc := NewNonInteractiveDeferredLoadingClientConfig(
		...
		&ConfigOverrides{ClusterInfo: clientcmdapi.Cluster{Server: mainUrl}})
